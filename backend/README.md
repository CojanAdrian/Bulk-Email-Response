...
Every route below is scoped to the caller's own loads (matched by the
`user_id` recorded on each load), **except** for users with `role: 'admin'`,
who see and can edit every user's loads, not just their own. See "Accounts
and roles" below for who has the admin role.
...
| POST | `/api/loads/upload` | `{"loads": [{ "load_number": string, ... other load fields }]}` | `200 {"inserted": number, "updated": number}`; `400 {"error": "loads must be an array"}` if `loads` isn't an array. Loads are upserted into the caller's own set only — see below for `load_number` uniqueness |
...
`role: 'admin'`. Admins see and can edit every user's loads across
`GET /api/loads`, `GET /api/loads/:id`, `PATCH /api/loads/:id`, and
`POST /api/loads/upload` — not just their own.
